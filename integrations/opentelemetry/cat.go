...
func (l traceListener) Before(ctx context.Context, name string, function api.Function) context.Context { //TODO implement me
	ctx, _ = l.tracer.Start(ctx, name, trace.WithSpanKind(trace.SpanKindInternal))
	return ctx
}

func (l traceListener) After(ctx context.Context, name string, function api.Function) {
...
func main() {
	...
	tp := sdktrace.NewTracerProvider(
		...
		sdktrace.WithResource(resource.NewWithAttributes(
			semconv.SchemaURL,
			semconv.ServiceNameKey.String("wazero"),
		)),
	)
	...
	tracer := tp.Tracer("wazero")

	r := wazero.NewRuntimeWithConfig(
		wazero.NewRuntimeConfigInterpreter().WithFunctionListener(&traceListener{tracer: tracer}))
	...
	cat, err := r.InstantiateModuleFromCodeWithConfig(catWasm, config.WithArgs("wasi", "./test.txt"))
	if err != nil {
		log.Fatal(err)
	}
	defer cat.Close()
